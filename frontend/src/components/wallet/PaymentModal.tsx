...
export default function PaymentModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md animate-in fade-in">
      <div className="relative max-w-md w-full rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-5">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Instant Credit Top-up Gateway</p>
            </div>
          </div>
          <button
            disabled={processing}
        ...
        {success ? (
          <div className="py-8 flex flex-col items-center justify-center text-center space-y-3">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-400 animate-bounce">
          ...
          <>
            ...
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 flex items-center justify-between">
              <div>
                <span className="text-[10px] uppercase font-bold text-emerald-400">Selected Plan</span>
            ...
            {/* Security Guarantee */}
            <div className="flex items-center gap-2 text-[11px] text-slate-400 bg-slate-950 p-2.5 rounded-xl border border-slate-800/80">
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                disabled={processing}
                onClick={onClose}
                className="flex-1 rounded-xl bg-slate-800 py-3 text-xs font-semibold text-slate-300 hover:bg-slate-700 transition"
              >
                Cancel
              </button>
              <button
                ...
                disabled={processing}
                onClick={handlePay}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-400 to-teal-300 py-3 text-xs font-extrabold text-slate-950 hover:from-emerald-300 hover:to-teal-200 transition shadow-lg shadow-emerald-500/20 active:scale-95 disabled:opacity-50"
              >
                {processing ? (
                  <>
